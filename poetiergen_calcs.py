...
from poetiergen_constants import NotDroppedList, json_unique_filepaths, json_bases_filepath, json_div_filepath, bases_useless_columns


# Div Card Tiering:

def calc_div_cards(min_price, json_data=None, exceptions=[]):
    json_data = json_data if json_data is not None else poepy.FileToJson(json_div_filepath)
...
def calc_uniques(min_price, json_data=None, exceptions=[]):
    json_data = json_data if json_data is not None else [poepy.FileToJson(js) for js in json_unique_filepaths]
    df = pd.concat((pd.DataFrame.from_dict(f) for f in json_data), ignore_index=True, sort=True)
    df = df[~df['baseType'].isin(exceptions)]
    df = df.query('name not in @NotDroppedList and links < 5') # doing df[~df['name'].isin(NotDroppedList)][df['links']<5] is slightly faster but looks worse v0v
...
def calc_item_bases(min_price, json_data=None):
    json_data = json_data if json_data is not None else poepy.FileToJson(json_bases_filepath)
    df = pd.DataFrame.from_dict(json_data)
    ...
    df = gvq.apply(fun)
    df.loc[:,'aValue'] = df.apply(lambda x: x['chaosValue']*x['confidence'] if x['confidence'] > 0.35 else 0, axis=1)
